...
# ///
"""
DW Build - Implementation phase for composable pipelines.

Implements a plan created by the planning phase.

Usage:
    ./dws/dw_build.py --dw-id abc12345
    ./dws/dw_build.py --dw-id abc12345 --model gpt-4o
"""
...
def main(dw_id: str, model: str, working_dir: str):
    """Implement the plan from a previous planning phase."""
    console = Console()
    agent_name = "builder"
    ...
    if not state:
        ...
        console.print("Run dw_plan.py first to create a plan.")
        sys.exit(1)
    ...
    info_table.add_row("Phase", "Build")
    ...
    console.print(
        Panel(
            info_table, title="[bold blue]Build Phase[/bold blue]", border_style="blue"
        )
    )
    console.print()

    # Render template
    rendered_prompt = load_and_render("implement", PLAN_FILE=state.plan_file)
    ...
    # Execute
    update_phase(state, "build", "running", agent_name=agent_name)
    ...
    try:
        ...
        if response.success:
            ...
            update_phase(
                state, "build", "failed", agent_name=agent_name, output=response.output
            )
            console.print(
                Panel(
                    response.output,
        ...
        update_phase(state, "build", "failed", agent_name=agent_name, output=str(e))
        console.print(
            Panel(
