...
# ///
"""
DW Review - Review phase for composable pipelines.

Reviews implementation against the plan specification.

Usage:
    ./dws/dw_review.py --dw-id abc12345
"""
...
def main(dw_id: str, model: str, working_dir: str):
    """Review the implementation against the plan specification."""
    console = Console()
    agent_name = "reviewer"
    ...
    info_table.add_row("Phase", "Review")
    ...
    console.print(
        Panel(
            info_table, title="[bold blue]Review Phase[/bold blue]", border_style="blue"
        )
    )
    console.print()

    # Render template
    rendered_prompt = load_and_render("review", PLAN_FILE=state.plan_file)
    ...
    # Execute
    update_phase(state, "review", "running", agent_name=agent_name)

    try:
        with console.status("[bold yellow]Reviewing implementation...[/bold yellow]"):
        ...
        if response.success:
            ...
            update_phase(
                state, "review", "failed", agent_name=agent_name, output=response.output
            )
            console.print(
                Panel(
                    response.output,
        ...
        update_phase(state, "review", "failed", agent_name=agent_name, output=str(e))
        console.print(
            Panel(
