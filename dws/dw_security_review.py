...
# ///
"""
DW Security Review - OWASP-based security review phase.

Classifies OWASP Top 10 categories and performs targeted security audit.

Usage:
    ./dws/dw_security_review.py --dw-id abc12345
"""
...
def main(dw_id: str, model: str, working_dir: str):
    """Run OWASP-based security review against the implementation."""
    console = Console()
    ...
    info_table.add_row("Phase", "Security Review")
    ...
    console.print(
        Panel(
            info_table,
            title="[bold blue]Security Review Phase[/bold blue]",
            border_style="blue",
        )
    )
    console.print()
    ...
    # Render template
    rendered_prompt = load_and_render("security-review", PLAN_FILE=state.plan_file)
    ...
    # Execute
    update_phase(state, "security_review", "running", agent_name=agent_name)

    try:
        with console.status("[bold yellow]Running security review...[/bold yellow]"):
        ...
        if response.success:
            ...
            console.print(
                Panel(
                    response.output,
                    title="[bold green]Security Review Complete[/bold green]",
                    border_style="green",
                    padding=(1, 2),
                )
            )
            ...
            console.print(
                Panel(
                    response.output,
                    title="[bold red]Security Review Failed[/bold red]",
                    border_style="red",
                    padding=(1, 2),
                )
            )
        ...
        update_phase(
            state, "security_review", "failed", agent_name=agent_name, output=str(e)
        )
        console.print(
            Panel(
